...
class CenterHead(BaseModule):
        ...
        Returns:
            Returns:
                tuple[list[torch.Tensor]]: Tuple of target including
                    ...
                    - list[torch.Tensor]: Heatmap scores. [(b x c x 28 x 128), ...]
                    - list[torch.Tensor]: Ground truth boxes.  [(b x 500 x 10), ...]
                    - list[torch.Tensor]: Indexes indicating the
                        position of the valid boxes. [(b x 500), ...]
                    - list[torch.Tensor]: Masks indicating which
                        boxes are valid. [(b x 500), ...]
        """
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        gt_bboxes_3d = torch.cat(
            ...
            dim=1).to(device) # 3D中心, 和其余部分 x,y,z,w,l,h,yaw,vx,vy 9个
        max_objs = self.train_cfg['max_objs'] * self.train_cfg['dense_reg']
        grid_size = torch.tensor(self.train_cfg['grid_size'])  # ?? 1024 * 1024 * 40
        pc_range = torch.tensor(self.train_cfg['point_cloud_range']) 
        voxel_size = torch.tensor(self.train_cfg['voxel_size'])  # 0.1 x 0.1 x 0.2

        feature_map_size = grid_size[:2] // self.train_cfg['out_size_factor']  # 128 x 128
        ...
        for idx, task_head in enumerate(self.task_heads):
            heatmap = gt_bboxes_3d.new_zeros(
                (len(self.class_names[idx]), feature_map_size[1],
                 feature_map_size[0])) # 1 x 128 x 128
                ...
                if width > 0 and length > 0:
                    ...
                    if self.with_velocity:
                        ...
                        anno_box[new_idx] = torch.cat([  
                            center - torch.tensor([x, y], device=device), # dx, dy 需要回归的
                            z.unsqueeze(0), box_dim,
                            torch.sin(rot).unsqueeze(0),
                            torch.cos(rot).unsqueeze(0),
                            vx.unsqueeze(0),
                            vy.unsqueeze(0) # # dx, dy, z, log(h), log(w), log(z), sin(yaw), cos(yaw), vx, vy
                        ])
                    else:
                        anno_box[new_idx] = torch.cat([
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            target_box = anno_boxes[task_id] # b x 500 x 10
            ...
            pred = self._gather_feat(pred, ind)  # 预测信息 挑出 与标签相同位置的信息 b x 500 x 10
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_size = preds_dict[0]['heatmap'].shape[0]      # b
            batch_heatmap = preds_dict[0]['heatmap'].sigmoid()  # b x c x 128 x 128

            batch_reg = preds_dict[0]['reg']    # b x 2 x 128 x 128
            batch_hei = preds_dict[0]['height'] # b x 1 x 128 x 128

            if self.norm_bbox:
                batch_dim = torch.exp(preds_dict[0]['dim'])  # b x 3 x 128 x 128
            else:
                batch_dim = preds_dict[0]['dim']

            batch_rots = preds_dict[0]['rot'][:, 0].unsqueeze(1)  # b x 1 x 128 x 128 sin(yaw)
            batch_rotc = preds_dict[0]['rot'][:, 1].unsqueeze(1)  # b x 1 x 128 x 128 cos(yaw)

            if 'vel' in preds_dict[0]:
                batch_vel = preds_dict[0]['vel']  # b x 2 x 128 x 128
            else:
                batch_vel = None
